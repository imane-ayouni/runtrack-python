...
class Ant:
    ...
    # Enfin la fourmille  se déplace vers le dortoir
    ...
    def go(self, a,x,steps):
        ...
        for i in range(len(self.rooms.name)):
            if (self.vestibule, self.rooms.name[i]) in self.hallways:
                ...
                if (self.rooms.name[i], self.rooms.name[i + 3]) in self.hallways:
                    self.rooms.size[i] == 0
                    steps += 1
                    self.rooms.size[i + 1] == 1
                    print("Step ", steps)
                    print("Ant: ", y, " ", self.rooms.name[i], " ---> ", self.rooms.name[i + 3])
                    print("------------")

                    if (self.rooms.name[i+3], self.dorm) in self.hallways:
                        self.rooms.size[i + 1] == 0
                        steps += 1
                        self.dormSize += 1
                        print("Step ", steps)
                        print("Ant: ", y, " ", self.rooms.name[i+3], " ---> ", self.dorm)
                        print("Remains in vestibule: ", self.ant, " ants")
                        print("------------")
                        if self.ant == 0:
                            break
